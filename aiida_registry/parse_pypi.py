...
def get_pypi_metadata(package_name: str,
                      parse_wheel=True) -> Optional[PypiData]:
    ...
    try:
        with requests.get(build_types['bdist_wheel'], stream=True) as download:
                ...
                with zipfile.ZipFile(Path(tmpdirname, 'wheel.whl')) as whl:
                    ...
                    for name in whl.namelist():
                        if name.endswith('.dist-info/entry_points.txt'):
                            entry_points_content = whl.read(name).decode(
                                'utf-8')
